...
- Keep registered-repository validation and signoff in local integration-checkout policy. Host policy applies only to unregistered repositories.
...
- Treat `.iq/config.json` as untracked local control-plane configuration in the registered integration checkout. Its absence means no validation and no signoff. Reject tracked policy.
